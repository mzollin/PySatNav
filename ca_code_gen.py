...
def ca_code_gen(sv):
    ...
    Returns:
    infinite C/A code sequence as generator for a single int (only 0/1)
    """
...
def lfsr(seed, taps):
    ...
    Arguments:
    seed -- initialisation seed as list of int (only 0/1)
    taps -- position of feedback taps as list of int (only 0/1)
    Returns:
    infinite LFSR output as generator for a single int (only 0/1)
    """
    state = seed
    while True:
        feedback = 0
